...
$("#matrix1").click(function () {
    ...
    $("#metaConfigTable3").empty();
    $("#metaConfigTable3").append(tabelajzing(matrix));
});
...
$("#compet").click(function () {
    ...
    if (matrix.length !== 1) {
            ...
            if (i != 0) {
                for (j = 0; j < matrix[i].length; j++) {
                    if (j != 0) {
                        ...
                        var valueInt = parseInt(matrix[i][j]);
                        if (valueInt !== 0) {
                            var splitted = value.split(",");
                            arr.push(parseInt(splitted[0]));
                            arr.push(parseInt(splitted[1]));
                        }
                    }
                }
            }
        ...
        document.getElementById("resultCompet").innerHTML =
            "X => " + parseFloat(arr[0]).toFixed(2) + " | Y => " + parseFloat(arr[1]).toFixed(2);
    }
});
...
function drawArrow(begin, end) {
    ...
    while (!valueIsEmpty(attr)) {
        attr = prompt("Enter the value (X,Y)");
    }
